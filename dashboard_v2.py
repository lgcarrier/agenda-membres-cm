...
with tabs[0]:
    ...
    if not aggregated_events:
        ...
        st.markdown(f"Pendant la période du **{start_date.strftime('%Y-%m-%d')}** au **{end_date.strftime('%Y-%m-%d')}**, "
                    f"il y a eu un total de **{total_events}** événements, couvrant **{unique_locations}** lieux et "
...
with tabs[3]:
    ...
    if not aggregated_events:
        ...
        df = pd.DataFrame(aggregated_events)
        # Create a meeting flag based on keywords
        ...
        # Determine top performers in each category
